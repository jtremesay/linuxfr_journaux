...
"[Pydantic AI uses a dependency injection system to provide data and services to your agent’s system prompts, tools and output validators.](https://pydantic.dev/docs/ai/core-concepts/dependencies/)" Aucune idée de comment traduire ça. Pydantic AI utilise un système d'injection de dépendances pour fournir des données et des services aux prompts système, outils et validateurs de sortie de votre agent ? Et en français ? Doit pouvoir passer data à functions appelés par pydantic AI pour stocker bordel à toi.
...
```shell
$ uv run dependency.py 
C'est du soleil ! Le temps à Montpellier est ensoleillé aujourd'hui. Enchaux avec Jojo ☀️📶
```
